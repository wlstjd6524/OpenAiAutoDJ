...
def generate_images_for_songs(csv_file, youtube_title, youtube_description):
    ...
    for i, row in df_playlist.iterrows():
        if row['mp3'] == 'Not found':
            ...
            print(f"[{i}] {row['Title']} by {row['Artist']} - MP3 not found, skipping image generation.")
        else:
            if is_cuda_available:
                ...
                try:
                    ...
                    image_file = None
                time.sleep(5)
            print(f"[{i}] {row['Title']} by {row['Artist']} - Generated image file: {image_file}")
            ...
            print(f"[{i}] {row['Title']} by {row['Artist']} - Info image file: {info_image_file}")
